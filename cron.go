...
func (c *Cron) AddJob(spec string, cmd Job, id string) error {
	return c.Schedule(Parse(spec), cmd, id)
}
...
func (entrySlice entries) pos(id string) int {
...
// search job entity
func (entrySlice entries) search(id string) *Entry {
...
// search job cron
func (c *Cron) Search(id string) *Entry {
	return c.entries.search(id)
}

// Schedule adds a Job to the Cron to be run on the given schedule.
func (c *Cron) Schedule(schedule Schedule, cmd Job, id string) error {
	entry := &Entry{
		Schedule: schedule,
		Job:      cmd,
		ID:     id,
	...
	if !c.running {
		i := c.entries.pos(entry.ID)
		if i != -1 {
			return errors.New("job is exist")
		}
